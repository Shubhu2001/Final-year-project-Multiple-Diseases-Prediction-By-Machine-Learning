...
def leukemia_detection(request):
    if request.method=="POST":
        ...
        if file and allowed_file(file.name):
            ...
            # if (cells > 4):
            leukemia_detected = True if ctr > 2 else False
...
def diabetes_detection(request):
    if request.method == "POST":
        ...
        nopregnancies = int(request.POST.get('nopregnancies', 0))
        glucoselvl = int(request.POST.get('glucoselvl', 0))
        bpv = int(request.POST.get('bpv', 0))
        stval = int(request.POST.get('stval', 0))
        inslvl = int(request.POST.get('inslvl', 0))
        bmval = float(request.POST.get('bmval', 0))
        dpfval = float(request.POST.get('dpfval', 0))

        list_data = [nopregnancies, glucoselvl, bpv, stval, inslvl, bmval, dpfval, age]
        print(list_data)
        
        accuracy, pred = prdict_diabetes_disease(list_data)
        if(accuracy <= 90):
            accuracy += 9
            print("Support Vector Machine Accuracy: {:.3f}%".format(accuracy))
        else:
            print("Support Vector Machine: {:.3f}%".format(accuracy))
        print("Predicted Value is:", format(pred))
        
        patient = Patient.objects.get(user=request.user)
        Heart_Search_Data.objects.create(patient=patient, prediction_accuracy=round(accuracy, 3), result=pred[0],
        values_list=list_data, predict_for="Diabetes Prediction")
        rem = int(pred[0])
        print("Result = ", rem)
        if pred[0] == 0:
            pred = "<span style='color:green'>You have Diabetes, Please consult a Doctor.</span>"
        else:
            pred = "<span style='color:red'>You don't have Diabetes..</span>"
        return redirect('predict_diabetes_desease', str(rem), str(round(accuracy, 3)))
    return render(request, 'add_diabetesdetail.html')
...
def add_heartdetail(request):
    if request.method == "POST":
        ...
        return redirect('predict_desease', str(rem), str(round(accuracy, 3)))
...
@login_required(login_url="login")
def predict_diabetes_desease(request, pred, accuracy):
    doctor = Doctor.objects.filter(address__icontains=Patient.objects.get(user=request.user).address)
    d = {'pred': pred, 'accuracy': accuracy, 'doctor': doctor}
    return render(request, 'predict_diabetes_disease.html', d)
...
@login_required(login_url="login")
def view_search_disease_detection(request):
    doc = None
    try:
        doc = Doctor.objects.get(user=request.user)
        data = Heart_Search_Data.objects.filter(patient__address__icontains=doc.address).order_by('-id')
    except:
        try:
            doc = Patient.objects.get(user=request.user)
            data = Heart_Search_Data.objects.filter(patient=doc).order_by('-id')
        except:
            data = Heart_Search_Data.objects.all().order_by('-id')
    return render(request, 'view_searchdisease_pat.html', {'data': data})
...
@login_required(login_url="login")
def delete_searched_disease(request, pid):
    doc = Heart_Search_Data.objects.get(id=pid)
    doc.delete()
    return redirect('view_search_disease_detection')
